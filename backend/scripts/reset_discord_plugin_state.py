#!/usr/bin/env python3
"""
Reset the Discord chatbot plugin's service/runtime state for rapid testing.
...
DEFAULT_DEFINITION_ID = "braindrive_discord_settings"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reset Discord plugin state (services + DB + shared files).")
    ...
    parser.add_argument(
        "--definition-id",
        default=None,
        help="Optional settings definition id to wipe (defaults to Discord settings).",
    )
...
async def run(args: argparse.Namespace) -> int:
    ...
    if args.definition_id:
        if args.definition_id not in definition_ids:
            definition_ids.append(args.definition_id)
    elif DEFAULT_DEFINITION_ID not in definition_ids:
        definition_ids.append(DEFAULT_DEFINITION_ID)
